...
describe("Package", () => {
  ...
  it("button should show version then become interactive", () => {
    cy.visit("/#/"); // preload
    cy.visit(VISIT_URL); // direct visit
    cy.get("button[data-testid=button-version-choice]:disabled") // the button should be disabled
      .getByText("react@16.3.0");
    cy.get("button[data-testid=button-version-choice]").should(
      "not.be.disabled"
  ...
  it("should open package.json tab", () => {
    cy.visit(VISIT_URL); // direct visit
    cy.getByTestId("packageJson-tab")
  ...
  ["DESKTOP", "MOBILE"].forEach(platform => {
    it(`[${platform}] should access version from title ${
      ...
      cy.visit(VISIT_URL); // direct visit
      cy.get("button[data-testid=button-version-choice]") // open list
        .click()
        .get(
